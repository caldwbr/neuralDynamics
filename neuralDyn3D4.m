...
%
%  CHANGE (this version): instead of one combined quad figure, each regime
%  is rendered in its OWN figure and saved as a separate JPG with generous
%  white space around the edges:
%        snicLC.jpg   snLC.jpg   subHopfLC.jpg   superHopfLC.jpg
clear; clc; close all;
...
%% =================== LC + STRUCTURE -- FOUR SEPARATE IMAGES ===================
% Each excitability regime is rendered in its OWN figure and saved as a
% separate JPG. The plotting axes is inset from the figure edges so every
% saved image keeps generous white space around the title, axis labels,
% and legend.
%
% Columns: { params, I-range, I_bif, panel title, output filename }
regimes = { ...
    p_SNIC, Ir_SNIC,  Ibif_SNIC, 'SNIC LC manifold',               'snicLC.jpg';    ...
    p_SN,   Ir_SN,    Ibif_SN,   'SN LC manifold',                 'snLC.jpg';      ...
    p_subH, [35, 65], Ibif_subH, 'Subcritical Hopf LC manifold',   'subHopfLC.jpg'; ...
    p_supH, Ir_supH,  Ibif_supH, 'Supercritical Hopf LC manifold', 'superHopfLC.jpg' };

fprintf('\nRendering LC+structure panels as four separate images...\n');
for r = 1:size(regimes,1)
    ...
    makeLCPanel(regimes{r,1}, regimes{r,2}, regimes{r,3}, axR, regimes{r,4});
    drawnow;

    % print() captures the FULL figure canvas (white margins included), so the
    % inset axes above is preserved as white space around the edges of the JPG.
    % (exportgraphics crops tight to the content and would remove the margins.)
    set(figR, 'PaperPositionMode', 'auto');
    print(figR, regimes{r,5}, '-djpeg', '-r300');
    fprintf('  Saved %s\n', regimes{r,5});
end
fprintf('\nDone: snicLC.jpg, snLC.jpg, subHopfLC.jpg, superHopfLC.jpg\n');
%% ============================ FUNCTIONS ============================
...
function makeLCPanel(p, Irange, Ibif, ax, name)
    ...
    % Lift title slightly so the legend can sit beneath it without overlap
    title(name,'FontSize',11,'Units','normalized','Position',[0.5 1.05 0]);
    % Legend INSIDE the panel, nudged down so it clears the title
    lgd = legend('FontSize',12,'Location','northeast','Interpreter','latex');
    drawnow;                              % force layout so Position is valid
    lgd.Position(2) = lgd.Position(2) - 0.045;   % move DOWN (clears title)
    lgd.Position(1) = lgd.Position(1) + 0.005;   % small nudge right
    set(ax,'Projection','perspective');
end
