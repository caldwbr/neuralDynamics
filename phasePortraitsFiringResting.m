...
%  Phase portraits in BOTH a firing (repetitive-spiking) state and a
%  resting (quiescent) state -- 8 images total, each saved separately with
%  generous white space around the edges:
%
%     Firing:   snFiring.jpg   snicFiring.jpg   subHopfFiring.jpg   superHopfFiring.jpg
%     Resting:  snResting.jpg  snicResting.jpg  subHopfResting.jpg  superHopfResting.jpg
%
%  Subcritical-Hopf panel fixes (unchanged from the originals):
%    (1) Streamlines that leave the plotting box break with NaN (no
%        horizontal connect-lines).
%    (2) Per-panel n-window: subHopf gets nmax=1.00 so the outer stable LC
%        fits inside the box.
%    (3) Limit-cycle search uses a far-amplitude seed list + long settling.
clear; clc; close all;
...
%% Diagnostics (console only; safe to ignore)
for Itest = 3:0.25:7
...
%% Injected-current values for each state
% FIRING (repetitive spiking)
If_SNIC = 15.0;  If_SN = 6.0;   If_subH = 51.0;  If_supH = 35.0;
% RESTING (quiescent)
Ir_SNIC =  2.0;  Ir_SN = 2.0;   Ir_subH = 40.0;  Ir_supH = 10.0;
%% =================== EIGHT SEPARATE IMAGES ===================
% Each row: { params, I, regimeName, output filename }.
% regimeName must read 'Subcritical Hopf' exactly so the wider n-window
% (nmax = 1.00) is used for those two panels.
jobs = { ...
    p_SNIC, If_SNIC,  'SNIC',                'snicFiring.jpg';     ...
    p_SN,   If_SN,    'SN',                  'snFiring.jpg';       ...
    p_subH, If_subH,  'Subcritical Hopf',    'subHopfFiring.jpg';  ...
    p_supH, If_supH,  'Supercritical Hopf',  'superHopfFiring.jpg';...
    p_SNIC, Ir_SNIC,  'SNIC',                'snicResting.jpg';    ...
    p_SN,   Ir_SN,    'SN',                  'snResting.jpg';      ...
    p_subH, Ir_subH,  'Subcritical Hopf',    'subHopfResting.jpg'; ...
    p_supH, Ir_supH,  'Supercritical Hopf',  'superHopfResting.jpg' };

fprintf('\nRendering 8 phase portraits as separate images...\n');
for r = 1:size(jobs,1)
    figR = figure('Color','white','Position',[100 100 900 780]);

    % Inset axes -> leaves white space on all four edges of the saved image.
    axR = axes('Parent', figR, 'Position', [0.14 0.13 0.70 0.72]);

    plotOneRegime(jobs{r,1}, jobs{r,2}, axR, jobs{r,3});
    drawnow;

    % print() captures the FULL white figure canvas (margins included), so the
    % inset axes above is preserved as white space around the edges of the JPG.
    % (exportgraphics would crop tight to the content and remove the margins.)
    set(figR, 'PaperPositionMode', 'auto');
    print(figR, jobs{r,4}, '-djpeg', '-r300');
    fprintf('  Saved %s\n', jobs{r,4});
end
fprintf('\nDone: 8 images written (4 firing + 4 resting).\n');
%% ============================ FUNCTIONS ============================
