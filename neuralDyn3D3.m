...
%% =================== LC + STRUCTURE QUAD ===================
% Bigger figure + explicit tight panel positions (less gap between the 4).
figLC = figure('Color','white','Position',[80 80 1500 1150]);

% Panel geometry [left bottom width height], normalized. Larger panels, smaller gaps.
% Bottom row sits a touch lower (Ty) so titles + legends have headroom.
axW = 0.41; axH = 0.41;
Lx = 0.055; Rx = 0.545;     % left / right column origins
By = 0.555; Ty = 0.055;     % top / bottom row origins
axTL = axes('Position',[Lx By axW axH]);
axTR = axes('Position',[Rx By axW axH]);
axBL = axes('Position',[Lx Ty axW axH]);
axBR = axes('Position',[Rx Ty axW axH]);

fprintf('\nRendering LC+structure panels...\n');
makeLCPanel(p_SNIC, Ir_SNIC,   Ibif_SNIC, axTL, 'SNIC LC manifold');                fprintf('  SNIC done.\n');
makeLCPanel(p_SN,   Ir_SN,     Ibif_SN,   axTR, 'SN LC manifold');                  fprintf('  SN done.\n');
makeLCPanel(p_subH, [35, 65],  Ibif_subH, axBL, 'Subcritical Hopf LC manifold');   fprintf('  subH done.\n');
makeLCPanel(p_supH, Ir_supH,   Ibif_supH, axBR, 'Supercritical Hopf LC manifold'); fprintf('  supH done.\n');

% ---- Save high-res image directly (no screenshot) ----
exportgraphics(figLC, 'bifurcation_quad.png', 'Resolution', 600);
fprintf('\nSaved bifurcation_quad.png at 600 DPI.\n');
